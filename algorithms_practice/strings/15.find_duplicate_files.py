...
What is the time complexity of your modified solution? What is the most time-consuming part and memory consuming 
...
#             start = file.index('(')
#             duplicates[file[start:]].append(path + '/' + file[:start])
#
#     result = []
#     for path in duplicates.values():
#         if len(path) > 1: result.append(path)
#
#     return result
from algorithms3.strings import find_duplicate_files
a=["root/a 1.txt(abcd) 2.txt(efgh)", "root/c 3.txt(abcd)", "root/c/d 4.txt(efgh)", "root 4.txt(efgh)"]

print(find_duplicate_files.duplicate_files(a))
